    ...
    <>
      ...
      >
        <Link href={`/boards/${board.id}`} className="block group">
          <div className="relative rounded-2xl overflow-hidden transition-all duration-500 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-black/40">
            ...
            <div className="relative" style={{ background: color.surface }}>
              ...
              <div
                ...
                style={{ background: `radial-gradient(ellipse at 50% 35%, ${color.glow}, transparent 65%)` }}
              />

              {/* Glass border */}
              <div className="absolute inset-0 rounded-2xl border border-white/[0.06] group-hover:border-white/[0.14] transition-colors duration-500 pointer-events-none" />

              {/* ── Identity ── */}
              <div className="pt-10 pb-8 px-5 flex flex-col items-center text-center relative z-10">
                {emoji ? (
                  <span className="text-[46px] leading-none select-none mb-5 transition-transform duration-500 ease-out group-hover:scale-110 drop-shadow-lg">
                    {emoji}
                  </span>
                ) : (
                  <div className="relative mb-5 transition-transform duration-500 ease-out group-hover:scale-110">
                    {/* Glow behind sparkle */}
                    <div
                      className="absolute inset-0 blur-2xl opacity-30 group-hover:opacity-50 transition-opacity duration-700 rounded-full"
                      style={{ background: color.hex, transform: 'scale(2.5)' }}
                    />
                    <SparkleIcon
                ...
                {board.description && (
                  <p className="text-[11px] text-white/20 mt-2 truncate max-w-full leading-relaxed">
                    {board.description}
                  </p>
                )}
              </div>
              ...
              <div className="px-5 pb-4 flex items-center justify-between relative z-10">
                ...
                <span className="text-[10px] text-white/12 tabular-nums">
              ...
              <div className="absolute top-3 right-3 z-20" ref={menuRef}>
                <button
                  ...
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-white/10 hover:text-white/50 hover:bg-white/[0.06] transition-all duration-300 cursor-pointer opacity-0 group-hover:opacity-100"
                >
                ...
                {menuOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-1 w-40 bg-[#161616]/95 backdrop-blur-xl border border-white/[0.08] rounded-xl shadow-2xl shadow-black/60 overflow-hidden z-50 py-1"
                  >
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                      ...
                      className="flex items-center gap-2.5 w-full px-3.5 h-8 text-[12px] text-white/50 hover:text-white hover:bg-white/[0.06] transition-all cursor-pointer"
                    >
